...
- Agents MUST keep GymPrimer aligned with `CONSTITUTION.md` and `VISION.md`: a Markdown-first, citation-based exercise, movement, and training-literacy primer for a general audience, not a workout planner, clinical product, AI coach, hosted app, or video-first media project. Expanded pattern, condition, programming-literacy, program-example, or advanced exercise-literacy content MUST NOT rely on a broader direction until ADRs, specs, architecture, plans, tests, and validation are updated in source-of-truth order.
- Agents SHOULD make content practically helpful by foregrounding the reader's pain point and general recommended next steps when the approved spec allows it. Safety routing is still required for health-adjacent pages, but it SHOULD use calm plain language and SHOULD NOT overpower the recommended exercise or education menu.
