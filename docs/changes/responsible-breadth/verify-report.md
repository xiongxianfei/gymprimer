...
| Visual/media boundary and provenance inheritance | RB-T15, RB-MP7; Markdown-first media tests | expanded pages, `media/` rules inherited | Original proof passed. Supersession note: the later `apt-pattern-architecture` follow-up replaced the APT alignment visual with a reviewed raster image and recorded raster provenance in `media/PROVENANCE.md`. | pass |
