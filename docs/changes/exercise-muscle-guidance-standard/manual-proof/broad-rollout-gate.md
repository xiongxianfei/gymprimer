...
| `exercises/seated-row.md` | post-PR follow-up added role table and muscle-attention image | keep updated seated-row image/guidance if code review accepts the follow-up; otherwise revert to pre-follow-up text-only state | The follow-up remains a single-page upper-body pull slice with page-local source surface, prompt record, provenance row, and image-alignment proof. | post-PR review |
